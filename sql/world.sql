...
COPY countries_languages (countrycode, "language", isofficial, percentage) FROM stdin;
...
ALTER TABLE ONLY countries_languages
    ADD CONSTRAINT countries_languages_pkey PRIMARY KEY (countrycode, "language");

ALTER TABLE ONLY countries
    ADD CONSTRAINT country_capital_fkey FOREIGN KEY (capital) REFERENCES cities(id);

ALTER TABLE ONLY countries_languages
    ADD CONSTRAINT countries_languages_countrycode_fkey FOREIGN KEY (countrycode) REFERENCES countries(code);
...
-- Additional indexes for Mammoth testing

CREATE INDEX ON cities (countrycode, district);
CREATE INDEX ON countries_languages("language");
CREATE INDEX ON countries_languages("language"); -- duplicate index for testing index dupes detection
CREATE INDEX ON countries_languages("language",isofficial);
